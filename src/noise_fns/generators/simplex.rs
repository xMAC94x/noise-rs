...
const SKEW_2D: f64 = 0.366_025_403_784_438_646_763_723_170_752_94; // 0.5 * (sqrt(3.0) - 1.0)
const UNSKEW_2D: f64 = 0.211_324_865_405_187_117_745_425_609_749_02; // (3.0 - sqrt(3.0)) / 6.0
const SKEW_3D: f64 = 1.0 / 3.0;
const UNSKEW_3D: f64 = 1.0 / 6.0;
const SKEW_4D: f64 = 0.309_016_994_374_947_424_102_293_417_182_82; // (sqrt(5.0) - 1.0) / 4.0
const UNSKEW_4D: f64 = 0.138_196_601_125_010_515_179_541_316_563_44; // (5.0 - sqrt(5.0)) / 20.0
...
impl NoiseFn<[f64; 2]> for Simplex {
    fn get(&self, point: [f64; 2]) -> f64 {
        ...
        let s = (point[0] + point[1]) * SKEW_2D;

        let floored = math::to_isize2(math::map2(math::add2(point, [s; 2]), f64::floor));

        let t = (floored[0] + floored[1]) as f64 * UNSKEW_2D;
        ...
        let corner2 = math::add2(math::sub2(distance, math::to_f64_2(offsets)), [UNSKEW_2D; 2]);

        let corner3 = math::add2(math::sub2(distance, [1.0; 2]), [2.0 * UNSKEW_2D; 2]);
...
impl NoiseFn<[f64; 3]> for Simplex {
    fn get(&self, point: [f64; 3]) -> f64 {
        ...
        fn surflet(gradient_index: usize, distance: [f64; 3]) -> f64 {
            let mut t = 0.6 - distance[0] * distance[0] - distance[1] * distance[1]
        ...
        let s = (point[0] + point[1] + point[2]) * SKEW_3D;

        let floored = math::to_isize3(math::map3(math::add3(point, [s; 3]), f64::floor));
        ...
        let corner2 = math::add3(math::sub3(distance, math::to_f64_3(offset1)), [UNSKEW_3D; 3]);
...
impl NoiseFn<[f64; 4]> for Simplex {
    fn get(&self, point: [f64; 4]) -> f64 {
        ...
        fn surflet(gradient_index: usize, distance: [f64; 4]) -> f64 {
            let mut t = 0.6 - distance[0] * distance[0] - distance[1] * distance[1]
                - distance[2] * distance[2] - distance[3] * distance[3];
            if t < 0.0 {
                0.0
            } else {
                t *= t;
        ...
        let s = (point[0] + point[1] + point[2] + point[3]) * SKEW_4D;

        let floored = math::to_isize4(math::map4(math::add4(point, [s; 4]), f64::floor));

        let t = (floored[0] + floored[1] + floored[2] + floored[3]) as f64 * UNSKEW_4D;
        ...
        if distance[0] > distance[1] { rank_x += 1;} else { rank_y += 1;};
        if distance[0] > distance[2] { rank_x += 1;} else { rank_z += 1;};
        if distance[0] > distance[3] { rank_x += 1;} else { rank_w += 1;};
        if distance[1] > distance[2] { rank_y += 1;} else { rank_z += 1;};
        if distance[1] > distance[3] { rank_y += 1;} else { rank_w += 1;};
        if distance[2] > distance[3] { rank_z += 1;} else { rank_w += 1;};

        let mut offset1 = [0; 4];
        let mut offset2 = [0; 4];
        let mut offset3 = [0; 4];

        if rank_x >= 3 {offset1[0] = 1};
        if rank_y >= 3 {offset1[1] = 1};
        if rank_z >= 3 {offset1[2] = 1};
        if rank_w >= 3 {offset1[3] = 1};

        if rank_x >= 2 {offset2[0] = 1};
        if rank_y >= 2 {offset2[1] = 1};
        if rank_z >= 2 {offset2[2] = 1};
        if rank_w >= 2 {offset2[3] = 1};

        if rank_x >= 1 {offset3[0] = 1};
        if rank_y >= 1 {offset3[1] = 1};
        if rank_z >= 1 {offset3[2] = 1};
        if rank_w >= 1 {offset3[3] = 1};

        let offset4 = [1; 4];

        let corner2 = math::add4(math::sub4(distance, math::to_f64_4(offset1)), [UNSKEW_4D; 4]);
        ...
        let corner5 = math::add4(math::sub4(distance, [1.0; 4]), [4.0 * UNSKEW_4D; 4]);
